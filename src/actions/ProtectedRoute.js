...
const ProtectedRoute = ({ auth, type }) => {
  ...
  if(auth.email_verified === 1 && auth.is_subscribed === 0){
    return type === "unsub" ? <Outlet /> : <Navigate to="/vsfintech/404" replace />;
  }else if(auth.email_verified === 1 && auth.is_subscribed === 1){
    return type === "sub" ? <Outlet /> : <Navigate to="/vsfintech/404" replace />;
  }else{
    return <Navigate to="/vsfintech/login" replace />;
  }
};
